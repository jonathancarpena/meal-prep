...
function MealGrid({ data }) {
  ...
  const MealType = {
    breakfast: {
      icon: (
        <MdBreakfastDining className='text-white inline-block text-[2rem]' />
      ),
      color: 'bg-orange-400',
    },
    lunch: {
  ...
  return (
    <div
      className={`grid grid-cols-1 md:grid-cols-2 md:max-w-[650px] lg:grid-cols-3 justify-items-center place-items-center gap-7  lg:max-w-[1500px] mx-auto `}>
      {data.map((item, idx) => (
        // Product
        <div
          onClick={() =>
          ...
          <div
            className={`${
              !item.img ? 'bg-neutral-300' : 'bg-white'
          ...
          <div className='bg-white h-[70px] lg:h-[90px] flex items-center justify-center absolute bottom-0 w-full'>
            <span className='uppercase font-semibold text-neutral-700 lg:text-2xl'>
              {item.name}
            </span>
          </div>
