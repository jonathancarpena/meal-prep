    ...
    <div>
        ...
        <Swiper
          ...
          {similarMeals.map((item, idx) => (
            <SwiperSlide
              key={`Meal-${idx}`}
              onClick={() =>
                handleNavigate(`/meals/${replaceSpaces(item.name)}/${item._id}`)
              }
      ...
      <div className='lg:hidden relative mt-[2rem]   rounded-2xl mb-10'>
        <Swiper
          ...
          loop={true}
          className={` md:w-[400px] bg-white rounded-2xl `}>
          {similarMeals.map((item, idx) => (
            <SwiperSlide
              key={`Meal-${idx}`}
              onClick={() =>
              ...
              className='mb-12 pt-10  cursor-pointer flex flex-col justify-center items-center '>
              <div className='w-[200px] h-[200px] bg-neutral-200 flex items-center justify-center rounded-xl'>
                {item.img ? (
                  <Image
                    src={item.img}
